...
In the previous sections you installed all tools, connected the [Alif E8 DevKit](https://alifsemi.com/support/kits/ensemble-e8devkit/), and configured [FWAuto](https://fwauto.ai/). In this section you build the SLM firmware from source and flash it to the board. You do this manually first to understand the process, then use FWAuto's `/build` and `/deploy` commands.
...
With the firmware deployed, you are ready to start the web server and interact with the model from your browser.
